...
def main():
    ...
    #with S3Connector(s3) as conn:

    for bucket in s3.buckets.all():
        print(bucket.name)
    dt = date.today() + timedelta(days=1)
    with open('C:/Users/raikh/VscodeProjects/flask_app/s3/my-bucket-eks-policy.json', 'rb') as data:
        
        
        for i in range(100):
            
            tm = time.time()
            key = '%s/%s/my-bucket-eks-policy.json'%(dt, tm)
            s3.Bucket('my-bucket-raikhovn').put_object(Key=key, Body=data)  
    # List objects
    
    for o in S3Connector.list(s3.Bucket('my-bucket-raikhovn'), "%s/"%dt):
        print(o.key)          
